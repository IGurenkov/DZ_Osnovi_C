...
TCoord getHumanCoord(const TGame& g)
{
    ...
    {        
        HSTREAM str; //поток
        string FileName = "click.mp3"; //аудио файл
        str = BASS_StreamCreateFile(FALSE, PCHAR(FileName.c_str()), 0, 0, 0); //загрузка файла в поток

        HANDLE coordXY = GetStdHandle(STD_OUTPUT_HANDLE);
        SetConsoleTextAttribute(coordXY, 11);
        cout << "                                  " << "ENTER X: ";
        cin >> c.x && BASS_ChannelPlay(str, true);
        cout << "                                  " << "ENTER Y: ";
        cin >> c.y && BASS_ChannelPlay(str, true);
        SetConsoleTextAttribute(coordXY, 7);
...
{ 
    ...
    do
    {        
        PlaySound(TEXT("BackMusic.wav"), NULL, SND_FILENAME | SND_ASYNC);
        ...
        do
        {
            ...
            {                  
                ...
                Sleep(500);
                str = BASS_StreamCreateFile(FALSE, PCHAR(FileName.c_str()), 0, 0, 0);
        ...
        cout << "                 " << "PLAY AGAIN? WRITE 'Y' FOR YES OR ANOTHER FOR NO: ";
        cin >> answer;        
